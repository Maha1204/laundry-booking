...
def init_db():
    try:
        ...
        if result['count'] == 0:
            # Öka storleken på 'time' i UNIQUE KEY om den tidigare var VARCHAR(10)
            cursor.execute('''
                ALTER TABLE bookings 
                MODIFY time VARCHAR(100) NOT NULL
            ''')
            conn.commit()
...
def create_booking():
    try:
        ...
        # Kontrollera om tiden redan är bokad (använder nu det fullständiga tidsintervallet)
        cursor.execute(
        ...
        if cursor.fetchone():
            cursor.close()
            conn.close()
            return jsonify({'error': f'Tiden {data["time"]} är redan bokad detta datum'}), 400
        ...
        # Fånga Unique key error om det ändå skulle ske (redundant med checken ovan, men bra att ha)
        if 'Duplicate entry' in str(e):
            return jsonify({'error': 'Denna tid är redan bokad (Databasfel)'}), 400
        return jsonify({'error': str(e)}), 500
...
# Hämta bokningar för specifikt datum (Används inte i nya frontend men behålls)
